...
<v-toolbar flat color="white">
		...
		<v-spacer></v-spacer>
		<v-btn v-shortkey="['alt', 'r']" @shortkey="findBeneficiaries()" color="primary" class="mb-2" icon @click="findBeneficiaries()">
			<v-icon>refresh</v-icon>
		</v-btn>
		<v-dialog v-model="dialog" persistent max-width="700px">
			<v-btn v-shortkey="['alt', 'n']" @shortkey="dialog=true" slot="activator" color="primary" icon class="mb-2">
				<v-icon>add</v-icon>
			</v-btn>
			<v-card>
				...
				<form @submit.prevent="submit">
					<v-container grid-list-sm class="pa-4">
						<v-layout row wrap>
							...
							<v-flex xs12 sm6>
								<v-text-field label="Cédula" required v-model="editedItem.dni" />
							</v-flex>
							<v-flex xs12 sm6>
