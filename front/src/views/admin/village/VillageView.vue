<template>
  <div>
    <v-toolbar flat color="white">
      <v-toolbar-title>Comunidades</v-toolbar-title>
      ...
      <v-dialog v-model="dialog" persistent max-width="400px">
        ...
        <v-card>
          <v-card-title class="primary lighten-3 white--text">
            <span class="headline" v-if="editedIndex === -1">Nueva Comunidad</span>
            <span class="headline" v-else>Editar Comunidad</span>
          </v-card-title>
          <form @submit.prevent="submit">
            <v-container grid-list-sm class="pa-4">
              <v-layout row wrap>
                <v-flex xs12>
                  <v-text-field label="Nombre de la comunidad" required v-model="editedItem.name"/>
                </v-flex>
              </v-layout>
            </v-container>
            <v-card-actions>
...
<script lang="ts">
import VillageView from '@/views/admin/village/VillageView.ts'
